...
def employee_details(empid):
    ...
    leaves_count_query = db.select(func.count(models.employee .empid)).join(
        models.leaves, models.employee .empid == models.leaves.empid).where(models.employee  .empid == empid)
    leaves = db.session.execute(leaves_count_query).scalar()

    max_leaves_query = db.select(models.designation.max_leaves).where(
        models.designation.jobid == models.employee  .title_id, models.employee  .empid == empid)
